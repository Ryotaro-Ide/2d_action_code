...
private void HitFloor()
{
    ...
    public void DamageHP(int amount,GameObject enemy ,bool isAttackDirect)
    {
        if(gameObject!=null){
        int damage=amount;
        if(_guard.IsDamageEase){
            GuardHP(amount,enemy,gameObject,isAttackDirect);
            return;
        }else if(IsBarrierExpand){
            BarrierHP(amount,enemy,gameObject,isAttackDirect);
            return;
        }
        HPChange(damage);
        ...
        if(_hp>0){
            ...
            StartCoroutine(_kB.InvicibleTime(gameObject));
        }else{
            GameOver();
        }
        }
